...
def ln_to_n(mean_ln, sd_ln):
    """
    This is a function used to convert between log-normal and normal distributions.
    """
    term = 1.0 + (sd_ln ** 2) / (mean_ln ** 2)
    mean_n = np.log(mean_ln / np.sqrt(term))
    std_n = np.sqrt(np.log(term))
    return mean_n, std_n

def n_to_ln(mean_n, sd_n):
    """
     This is a function used to convert between normal and log-normal distributions.
    """
    variance_ln = (np.exp(sd_n ** 2) - 1.0) * np.exp(2.0 * mean_n + sd_n ** 2)
    mean_ln = np.exp(mean_n + sd_n ** 2 / 2.0)
    sd_ln = np.sqrt(variance_ln)
    return mean_ln, sd_ln
...
def rnd_state_serialize(experiment):
    """
    ...
    json.dump(save_state, open(f"{experiment}_rnd_state.json", "w"))

def rnd_state_deserialize(experiment):
    """
     The function deserializes the state of the NumPy random number generator from the 
     "rnd_state.json" file.
    """
    # read the serialized state from the "rnd_state.json" file 
    tmp_state = json.load(open(f"{experiment}_rnd_state.json", "r"))
