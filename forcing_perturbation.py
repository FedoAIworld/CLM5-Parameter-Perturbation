...
def perturb_nc_file(year, month, iensemble):
    ...
    fname = ("/home/fernand/JURECA/CLM5_DATA/inputdata/atm/datm7/trial_22/DE-HoH/local/" +
            str(year) + "-" + str(month).zfill(2) + ".nc")
    outname = ("/home/fernand/JURECA/CLM5_DATA/inputdata/atm/datm7/trial_22/DE-HoH/local/" +
               "Ensemble/real_" + 
    ...
    with nc.Dataset(fname) as src, nc.Dataset(outname, "w") as dst:
        ...
        rnd  = np.random.multivariate_normal(mean, correl, dim_time*dim_lat*dim_lon)
        ...
        perturbations[:, 2] = rnd[:, 2] * sd[2]
        perturbations[:, 3] = rnd[:, 3] * sd[3]
        perturbations[:, 4] = np.exp(mean[4] + sd[4] * rnd[:, 4])
        perturbations[:, 5] = rnd[:, 5] * sd[5]
